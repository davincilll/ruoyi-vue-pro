...
public class InvocableHandlerMethod extends HandlerMethod {
    ...
    protected Object[] getMethodArgumentValues(Message<?> message, Object... providedArgs) throws Exception {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            args[i] = HandlerMethod.findProvidedArgument(parameter, providedArgs);
            if (args[i] != null) {
                continue;
            }
            if (!this.resolvers.supportsParameter(parameter)) {
                throw new MethodArgumentResolutionException(
                        message, parameter, HandlerMethod.formatArgumentError(parameter, "No suitable resolver"));
            }
            try {
                ...
                if (logger.isDebugEnabled()) {
                    ...
                    if (exMsg != null && !exMsg.contains(parameter.getExecutable().toGenericString())) {
                        logger.debug(HandlerMethod.formatArgumentError(parameter, exMsg));
                    }
                }
                throw ex;
            }
